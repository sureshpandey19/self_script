...
echo "SELECT 0,p.completestatus as process_status FROM process  p WHERE p.completestatus =1 UNION SELECT 0,tp.pick_flag as proxy_status FROM transaction_proxy  tp WHERE tp.pick_flag = 1 UNION SELECT 0,tv.pick_flag as vod_status FROM transaction_vod tv WHERE tv.pick_flag = 1 UNION SELECT 0,p.completestatus FROM process p;" | mysql $dbname -u $dbuser -p$dbpasswd 2>/dev/null > $process


if [ $a == `awk 'NR == 2 {print $2}' $process` ]
then
   echo "mam operation process is running so not restarting tmux!"
else
	 echo "There is no any mam operation process running so restarting tmux..."
