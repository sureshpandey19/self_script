#!/bin/bash
LOCATION[1]="/video/txmaster"
LOCATION[2]="/tx_hits_movies"
DESTINATION="/video/txproxy"
CHECKLOCATION="/video/txproxy"
DONELOCATION="${LOCATION[$COUNTER]}/video/"
MOVE="0"
TIMESTAMP_FILENAME="TIMESTAMP_Do_Not_Delete4.txt"
KEYFRAME_FLAG="0"
TOTAL_NO_OF_LOCATIONS=1;
INVENTORY_NAME="FILES_LIST4.txt"
		...
		sed -i "\|$FILE|d" $LOG_LOCATION/"$INVENTORY_NAME";
		   ...
		   PROXYFILENAME="$PROXYFILE".jpg
							...
							if [ ! -f "$DESTINATION/$PROXYFILENAME" ] && [ ! -f "$CHECKLOCATION/$PROXYFILENAME" ]; then
								...
								sleep 2 
								/usr/local/bin/ffmpeg -n -i "$FILE" -ss 00:00:29.000 -vframes 1 $DESTINATION/"$PROXYFILENAME"
