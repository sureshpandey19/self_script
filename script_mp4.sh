		...
		sed -i "\|$FILE|d" $LOG_LOCATION/"$INVENTORY_NAME";
		   ...
		   PROXYFILENAME="$PROXYFILE".mp4
							...
							if [ ! -f "$DESTINATION/$PROXYFILENAME" ] && [ ! -f "$CHECKLOCATION/$PROXYFILENAME" ]; then
								...
								sleep 2 
								/usr/local/bin/ffmpeg -i "$FILE" -y -r 25 -b:a 64k -b:v 1024k  -c:v libx264 -preset veryfast  -s 620:460  -map 0 -strict -2  $DESTINATION/"$PROXYFILENAME"
